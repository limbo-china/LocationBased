...
public class GetTraceData {
	...
	static {
			...
			PropertyConfigurator.configure("log4j.properties");
				logger = Logger.getLogger(GetTraceData.class.getName());
		try {
	...
	public void query(){
		...
		try{
			...
			inputStream = new FileInputStream(phonefilename);
			...
			while(sc.hasNext()){
				...
				msisdntmp="C_MSISDN="+"86"+s+" or "+msisdntmp;
			   	
			}
			
			//System.out.println(msisdntmp.substring(0,msisdntmp.length()-3));
			try{
...
//					BufferedWriter bufferWritter = new BufferedWriter(fileWritter);
					...
					sql= "select * from T_LBS_TRACE_HISTORY where C_TIMESTAMP >=unix_timestamp('2018-03-09 00:00:00') and  C_TIMESTAMP < unix_timestamp('2018-04-10 00:00:00') and ("+msisdntmp.substring(0,msisdntmp.length()-3)+") order by C_MSISDN desc;";	
