...
public class DataDispatchHandler extends AbstractHandler {
	...
	public static DataDispatchHandler getDataDispatchHandler() {
		...
		try {
			...
			// GenericArray docSet = new
			// GenericData.Array<GenericRecord>(batchSize,
	...
	public void handle(String string, Request baseRequest,
		...
		logger.info("receive request from " + remoteHost + ":" + remotePort
				+ "@" + region + " and assigned id " + reqID);
		...
		try {
			logger.debug("req " + reqID
					+ ":retriving bussisness data for request  ...");
			...
			logger.debug("req " + reqID + ":length of bussisness data is "
					+ req.length);
		} catch (Exception ex) {
			String errInfo = "req " + reqID
					+ ":retrive bussiness data unsuccessfully for "
					+ ex.getMessage();
		...
		if (req == null || req.length == 0) {
			String warnInfo = "req "
					+ reqID
					+ ":retrive bussiness data unsuccessfully for content is empty";
		...
		try {
			...
			BinaryDecoder docsbd = new DecoderFactory().binaryDecoder(docsbis,
					null);
			...
			String errInfo = "req " + reqID
					+ ":decode bussiness data unsuccessfully for "
					+ ex.getMessage();
			logger.error(errInfo, ex);
			httpServletResponse.setStatus(HttpServletResponse.SC_OK);
			httpServletResponse.getWriter().println("-1\n" + errInfo);
		...
		try {
			...
			String errInfo = "req " + reqID
					+ ":wrong format of bussiness data,can't get docSchemaName";
			logger.error(errInfo, ex);
			httpServletResponse.setStatus(HttpServletResponse.SC_OK);
			httpServletResponse.getWriter().println("-1\n" + errInfo);
			return;
		...
		String docSchemaFullName = docSchemaName
				+ (region.isEmpty() ? "" : "@" + region);
		logger.debug("req " + reqID + ":doc shcema full name is "
				+ docSchemaFullName);

		try {
			Object bzSysSign = docsRecord.get("sign");
			GenericArray<?> docsSet = (GenericArray<?>) docsRecord
					.get("doc_set");

			if (docsSet == null) {
				String errInfo = "req " + reqID
						+ ":wrong format of bussiness data, doc_set is null";
				logger.error(errInfo);
				httpServletResponse.setStatus(HttpServletResponse.SC_OK);
				httpServletResponse.getWriter().println("-1\n" + errInfo);
			} else {
				if (docsSet.size() <= 0) {
					...
					GenericArray<?> docSet = (GenericArray<?>) docsRecord
							.get("doc_set");
					...
					if (docSchemaName.equals("t_cdr")) {
						...
						while (itor.hasNext()) {
							ByteArrayInputStream databis = new ByteArrayInputStream(
									((ByteBuffer) itor.next()).array());
							BinaryDecoder dataDecoder = new DecoderFactory()
									.binaryDecoder(databis, null);
							GenericRecord record = (GenericRecord) contentReader
									.read(null, dataDecoder);
							// System.out.println(record);

							SMetaData metaLog = new SMetaData();

							metaLog.setTimestamp(record.get("c_timestamp") == null ? 0
									: Long.parseLong(record.get("c_timestamp")
											.toString()));
							metaLog.setCdrType(record.get("c_cdrtype") == null ? 999
									: Integer.parseInt(record.get("c_cdrtype")
											.toString()));
							metaLog.setMsisdn(record.get("c_usernum") == null ? "Defalt"
									: record.get("c_usernum").toString());
							metaLog.setRelateNum(record.get("c_relatenum") == null ? "Defalt"
									: record.get("c_relatenum").toString());
							metaLog.setImsi(record.get("c_imsi") == null ? "Defalt"
									: record.get("c_imsi").toString());
							metaLog.setImei(record.get("c_imei") == null ? "Defalt"
									: record.get("c_imei").toString());
							metaLog.setLac(record.get("c_lac") == null ? "Defalt"
									: record.get("c_lac").toString());
							metaLog.setCi(record.get("c_ci") == null ? "Defalt"
									: record.get("c_ci").toString());
							metaLog.setUli(record.get("c_uli") == null ? "Defalt"
									: record.get("c_uli").toString());
							metaLog.setRegionCode(record.get("c_areacode") == null ? "Defalt"
									: record.get("c_areacode").toString());
							metaLog.setHomeCode(record.get("c_homecode") == null ? "Defalt"
									: record.get("c_homecode").toString());
							metaLog.setCdrContent(record.get("c_content") == null ? "Defalt"
									: record.get("c_content").toString());
							metaLog.setLngi(0.0);
							metaLog.setLati(0.0);
							metaLog.setSourceData(record.toString());// add by
																		// zhangyu;
							al.add(metaLog);
						}
						...
						logger.error("docSchemaName : " + docSchemaName
								+ " wrong!!!");
					}
			...
			String errInfo = "req " + reqID
					+ ":internal error for sending data unsuccessfully for "
					+ ex.getMessage();
			logger.error(errInfo, ex);
			httpServletResponse.setStatus(HttpServletResponse.SC_OK);
			httpServletResponse.getWriter().println("-1\n" + errInfo);
